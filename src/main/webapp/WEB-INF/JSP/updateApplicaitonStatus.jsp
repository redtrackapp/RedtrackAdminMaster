...
function onchangeAppId(selectObject) {
	 ...
	 //  alert(appID);
	  ...
	  document.getElementById('saveApplicaitonStatusForm').action = "/redtrackadmin/onchangeAppIdforStatus";
	  document.getElementById('saveApplicaitonStatusForm').submit(); 
	  }
...
<script >
$(document).ready(function() {
    $('#saveApplicaitonStatusForm').submit(function() {
      var inVal = $("#appstatus").val();
       
       if (inVal.length == 0) {
           alert("Status value can't be empty");
            $("#appstatus").focus();
...
<body>
		...
		<div id="content">	
			...
			<section id="main">
				<article>
					...
					<div style="margin-top: 22px; margin-bottom: 21px;" class="selectionboxdiv" id="mapoldappiddiv">
						<label style="font-size: 13px"> Existing applications Name :</label>&nbsp;&nbsp;&nbsp;
					
						<select style="width: 185px; height: 26px;" id="appid"  name="appid" onchange="onchangeAppId(this)">
						<!-- <select style="width: 185px; height: 26px;" id="appid"  name="appid"> -->
							<option value="" selected="selected">Please select application</option>
							<c:forEach items="${listofapptypes}" var="listofapptypes">
						        <option value="${listofapptypes.key}">${listofapptypes.value}</option>
						    </c:forEach>
						</select>
            ...
            <tr>
                ...
                <tr>
                	<input type="hidden" name= "appidhidden" value="<c:out value="${applicationStatusList1.appId}" />" />
                    <td><input type="text"  class="countryCode" style="width: 185px; height: 26px;"  id="applicationStatusList[${status1.index}].countryCode" name="applicationStatusList[${status1.index}].countryCode"  value="<c:out value="${applicationStatusList1.countryCode}" /> " readonly/></td>
                    <td><input type="text" class="unitId" style="width: 185px; height: 26px;"  id="applicationStatusList[${status1.index}].unitId" name="applicationStatusList[${status1.index}].unitId"  value="<c:out value="${applicationStatusList1.unitId}" /> "readonly/></td>
                    <td>
                    <input type="text" style="width: 185px; height: 26px;"  id="appstatus" name="applicationStatusList[${status1.index}].status" value="<c:out value="${applicationStatusList1.status}" />"/>
                    </td>
                </tr>
            </c:forEach>		
