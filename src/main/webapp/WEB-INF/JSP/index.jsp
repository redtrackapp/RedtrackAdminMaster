...
<body>
		...
		<div id="content">	
			...
			<form name="submitapplicationFormBean" id="submitapplicationFormBean" method="POST" action="/redtrackadmin/submitapplicationFormBean">
			<section id="main">
				<article>
					...
					<label id="processing" style="position:absolute; display:none">
						
						<img src="./resources/images/processbar.gif"/>
								<span style="color: red; padding-left: 6px;">Reteriving please wait ...</span>
					</label>
					</div>
							...
							<table style="width: 100%; height: 100%;">
								...
								<c:choose> 
								  <c:when test="${i%3 == 0}">
							  	      ...
							  	      <tr>
								        <td style="padding-left: 30px; padding-top: 5px;"><label><input type="checkbox" name="countries" <c:forEach items="${selectedcountries}" var="selectedcountries"> <c:if test="${fn:contains(listofcountries.key,selectedcountries)}">checked</c:if> 	</c:forEach> id="cntry_${listofcountries.key}" class="countrycboxcss" value="${listofcountries.key}">  &nbsp;${listofcountries.value}</label></td>
								  </c:when>
								  <c:otherwise>
								        <td style="padding-left: 30px; padding-top: 5px;""><label><input type="checkbox" name="countries" <c:forEach items="${selectedcountries}" var="selectedcountries"> <c:if test="${fn:contains(listofcountries.key,selectedcountries)}">checked</c:if> 	</c:forEach>id="cntry_${listofcountries.key}" class="countrycboxcss" value="${listofcountries.key}">  &nbsp;${listofcountries.value}</label></td>
								  </c:otherwise>
							...
							<table style="width: 100%; height: 100%;">
								...
								<c:choose> 
								  <c:when test="${i%3 == 0}">
									...
									<tr>
										<td style="padding-left: 30px; padding-top: 3px;"><label><input type="checkbox" name="businessUnits" <c:forEach items="${selectedBusinessunits}" var="selectedBusinessunits"> <c:if test="${fn:contains(listofbizunits.key,selectedBusinessunits)}">checked</c:if>	</c:forEach> id="bizunit_${listofbizunits.key}" class="bizunitcboxcss" value="${listofbizunits.key}">  &nbsp;${listofbizunits.value}</label></td>
								  </c:when>
								  <c:otherwise>
								  		<td style="padding-left: 30px; padding-top: 3px;"><label><input type="checkbox" name="businessUnits" <c:forEach items="${selectedBusinessunits}" var="selectedBusinessunits"> <c:if test="${fn:contains(listofbizunits.key,selectedBusinessunits)}">checked</c:if>	</c:forEach>id="bizunit_${listofbizunits.key}" class="bizunitcboxcss" value="${listofbizunits.key}">  &nbsp;${listofbizunits.value}</label></td>
								  </c:otherwise>
